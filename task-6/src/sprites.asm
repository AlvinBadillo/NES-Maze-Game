...
lvl_progress: .res 1 
collision_tile: .res 1

arena: .res 1 ;var #20
timer: .res 1
final_time: .res 1

.exportzp player_x, player_y, pad1, tick_count, state, bg_x, bg_y, scroll, level_select, lvl_progress, arena, timer
...
.proc update_game
  ...
  CMP timer
  BNE keep_going
  ;If we enter this, it means time is up
  LDA #$FA 
  STA lvl_progress
keep_going:
...
.proc draw_countdown
;load second number
    LDA timer     ; Load the number you want to divide
    SEC                  ; Set carry flag for subtraction
    LDX #00               ; Initialize a counter for quotient
    
DIV_LOOP2:
    SBC #10              ; Subtract 10 from the accumulator
    BCC END_DIV_LOOP2     ; If carry flag is set, the result is negative, exit loop
    INX                  ; Increment quotient
    JMP DIV_LOOP2         ; Repeat the loop
    
END_DIV_LOOP2:
    SEC                  ; Set carry flag for addition
    ADC #10              ; Add 10 back to the accumulator to get the remainder
    TAX                  ; Transfer accumulator to X register (remainder)
    CMP #10              ; Check if remainder is 10
    BCS SKIP_ADJUSTMENT2  ; If remainder is 10 or greater, skip adjustment
    DEX                  ; Decrement remainder to correct for overshooting
    INX                  ; Increment quotient to compensate for decrementing remainder
SKIP_ADJUSTMENT2:
    ; The remainder is in the X register
    CLC
    ADC #$4E
    JSR draw_number2
    
    LDA timer     ; Load the number to divide
    SEC                  ; Set carry flag for subtraction
    LDX #00               ; Initialize a counter for quotient
    
DIV_LOOP:
    SBC #10              ; Subtract 10 from the accumulator
    BCC DIV_END          ; If carry flag is set, the result is negative, exit loop
    INX                  ; Increment quotient
    JMP DIV_LOOP         ; Repeat the loop
    
DIV_END:
    TXA                  ; Transfer quotient from X to A
    CLC
    ADC #$4E
    JSR draw_number         
    RTS
...
check_right:
  ...
  CLC
  ADC scroll
  ;if in second nametable substract 256
...
check_right_store_npx:  
  ...
  BEQ cont_check_right
  INC player_x
  ;when we move right, so does the scroll
...
not_zero:
  ...
  BEQ cont_check_left
  DEC player_x  ; If the branch is not taken, move player left
  ;when we move left, so does the scroll
  DEC scroll       ; substract one to our scroll variable each frame
  LDA scroll
  STA $2005        ; write the horizontal scroll count register
...
;Tile_type = (metatile_byte >> Actual_tile_shifts)
Get_tile_type:
